...
_dict_unit_lat_lon = {
    ...
    100: (_unit_lat_100, _unit_lon_100),
    10: (_unit_lat_10, _unit_lon_10),
}
...
def to_meshcode(lat, lon, level, astype):
    ...
    def meshcode_100(lat, lon):
        i = int(rem_lat_lv3(lat) / _unit_lat_100())
        j = int(rem_lon_lv3(lon) / _unit_lon_100())
        k = 5
        return meshcode_lv3(lat, lon) + str(i) + str(j) + str(k)

    def meshcode_10(lat, lon):
        k = int(rem_lat_100(lat) / _unit_lat_10())
        l = int(rem_lon_100(lon) / _unit_lon_10())
        return meshcode_100(lat, lon)[:-1] + str(k) + str(l)
    ...
    if level == 100:
        return astype(meshcode_100(lat, lon))

    if level == 10:
        return astype(meshcode_10(lat, lon))
