...
def _data_scalar():
    return [
        ...
        ({"lat": _lat_tokyo_tower, "lon": _lon_tokyo_tower, "level": 100}, "5339359906"),
        ({"lat": _lat_tokyo_tower, "lon": _lon_tokyo_tower, "level": 10}, "533935990623"),
        ...
        ({"lat": _lat_kyoto_tower, "lon": _lon_kyoto_tower, "level": 100}, "5235368057"),
        ({"lat": _lat_kyoto_tower, "lon": _lon_kyoto_tower, "level": 10}, "523536805704"),
    ]
...
def _data_vector(num_elements=10):
    return [
        (
            {
                "lat": np.array([_lat_tokyo_tower] * num_elements),
                "lon": np.array([_lon_tokyo_tower] * num_elements),
                "level": 1,
            },
            np.array(["5339"] * num_elements),
        ),
        (
            {
                "lat": np.array([_lat_tokyo_tower] * num_elements),
            ...
            np.array(["53392"] * num_elements),
        ),
        (
            {"lat": np.array([_lat_tokyo_tower] * num_elements), "lon": _lon_tokyo_tower, "level": 1},
            np.array(["5339"] * num_elements),
        ),
        (
            {"lat": np.array([_lat_tokyo_tower] * num_elements), "lon": _lon_tokyo_tower, "level": 40000},
            np.array(["53392"] * num_elements),
        ),
        (
            {"lat": _lat_tokyo_tower, "lon": np.array([_lon_tokyo_tower] * num_elements), "level": 1},
            np.array(["5339"] * num_elements),
        ),
        (
            {"lat": _lat_tokyo_tower, "lon": np.array([_lon_tokyo_tower] * num_elements), "level": 40000},
            np.array(["53392"] * num_elements),
        ),
    ]
...
def _data_performance(num_elements=1000000):
    return [
        (
            ...
            np.array([53393599212] * num_elements),
        ),
    ]
...
def _eq_scalars(actual, expect, input):
    eq_(
        actual,
        expect,
        msg="{} as {} but expectes {} as {} when {} is given.".format(
            actual, type(actual), expect, type(expect), input
        ),
    )
...
def _eq_vectors(actual, expect, input):
    np.testing.assert_array_equal(
        actual,
        expect,
        err_msg="{} as {} but expectes {} as {} when {} is given.".format(
            actual, type(actual), expect, type(expect), input
        ),
    )
